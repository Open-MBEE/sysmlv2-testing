...
|---|---|---|
| OpenSysML | nothing to clone — `opensysml==<version>` is a real PyPI package (a project dependency; `uv sync` installs it), and the matching `sysml-grpc` server binary is auto-fetched by `opensysml.binary.ensure_binary(...)` on first use | `OPENSYSML_VERSION` (optional — see `adapters/opensysml.py` for the default; `OPENSYSML_GRPC_VERSION`, the client's own variable, is honoured as a fallback). The client deliberately decouples its version from the gRPC server binary, so the **server** digest is what `svt:artifactDigest` pins |
